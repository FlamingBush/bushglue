...
| `breath` | JSON `{amplitude, period_ms, skew, enabled}` | idle oscillator config (on by default). The cycle runs BELOW the set point: swing ≈ `[target − 2·amplitude, target]` (entry at the valley phase — the flame dips from the set level, never above it). Amplitude = fraction of span (0–0.5, default 0.04), period 100–60000 ms (default 5000), skew = rise fraction of the cycle (0.05–0.95, default 0.5). NOTE: `bush-valve-sweep` disables breath for every run and does NOT restore it; after re-enabling, breathing re-engages on the next `target` that lands within the deadband (re-send the current position to kick it) |

// Emotional Fire - Unordered
//
// Pixelblaze v2/v3 1D pattern for individually addressable fixtures whose
// physical order is unknown or unimportant. Save the pattern under the name
// mapped by the bridge's set_pattern binding.
//
// Each fixture receives a stable golden-ratio phase. Emotional roles drift
// slowly between fixtures without assuming that adjacent indexes are neighbors.
// Actual bush/flame/pulse commands provide optional visual accents; vestigial
// flare and bigjet fields in sentiment are still ignored. Colors are calculated
// once per frame so Pixelblaze v2 can drive the complete configured pixel span.
...
function primaryHue(emotion) {
  if (emotion == 0) return .76
  if (emotion == 1) return .47
  if (emotion == 2) return .84
  if (emotion == 3) return .51
  if (emotion == 4) return .44
  if (emotion == 5) return .64
  return .69
}

function accentHue(emotion) {
  if (emotion == 0) return .97
  if (emotion == 1) return .49
  if (emotion == 2) return .94
  if (emotion == 3) return .60
  if (emotion == 4) return .39
  if (emotion == 5) return .70
  return .75
}
...
function calculatePixel(index) {
  // The irrational stride distributes phases evenly for any fixture count and
  // does not encode adjacency or physical position.
  var fixturePhase = frac(index * .618033)
  var selector = frac(fixturePhase + palettePhase)
  var emotion = emotionAt(selector)
  var localPulse = wave(motionPhase * .63 + fixturePhase)
  var movement = .10 + .12 * wave(basePhase + fixturePhase)
  var edge = localPulse

  if (sceneMode == 1) {
    // A distributed inhale: fixtures gather without implying a direction.
    var inhale = wave(phaseSeconds / 5 + fixturePhase * .22)
    movement = .18 + .62 * inhale + .20 * localPulse
    edge = max(inhale, localPulse)
  } else if (sceneMode == 2) {
    // Speech combines one global breath with stable per-fixture phases.
    var globalBreath = .35 + .65 * wave(motionPhase)
    movement = globalBreath * (.62 + .38 * localPulse)
    edge = localPulse
  } else if (sceneMode == 3) {
    // All fixtures exhale together, then their fixed phases stagger the tail.
    var releaseProgress = clamp(phaseSeconds / 3, 0, 1)
    var globalExhale = wave(releaseProgress - .25)
    var stagger = clamp(1 - phaseSeconds / max(5, decaySeconds) +
      fixturePhase * .16, 0, 1)
    movement = .10 + .72 * globalExhale + .18 * localPulse * stagger
    edge = max(globalExhale, localPulse * stagger)
  }

  var targetHue = primaryHue(emotion)
  var targetSaturation = primarySaturation(emotion)
  var accentAmount = clamp((edge - .70) / .30, 0, 1)
  ...
  var smoke = wave(basePhase + fixturePhase)
  var emberTwinkle = wave(basePhase * 3 + fixturePhase * 2.3)
  var value = .035 + .045 * smoke + .04 * emberTwinkle * emberTwinkle
  value = value + emotionLevel * (.05 + .24 * movement) *
    emotionalBrightness(emotion)

  // Valve pulses are visual accents only. Independent envelopes preserve
  // concurrent valves, while repeated pulses can only extend their influence.
  ...
  var coolMotion = .35 + .65 * wave(fixturePhase +
